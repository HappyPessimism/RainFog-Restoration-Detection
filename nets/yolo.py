...
from nets.backbone import Backbone,finBackbone,cleanBackbone, C2f, Conv#, DABackbone
from nets.yolo_training import weights_init
from nets.network import LarKPN, NAFNet
...
class finNAFYoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(finNAFYoloBody, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
        ...
        #   1024 * deep_mul, 20, 20
        #---------------------------------------------------#
        self.clean_backbone = cleanBackbone(base_channels, base_depth, deep_mul, phi, pretrained=pretrained)        
        self.backbone   = finBackbone(base_channels, base_depth, deep_mul, phi, pretrained=pretrained)
        self.rest = NAFNet()
        ...
        self.nl         = len(ch)
        # self.stride     = torch.zeros(self.nl)
        self.stride     = torch.tensor([256 / x.shape[-2] for x in self.backbone.forward(torch.zeros(1, 3, 256, 256))[1:]])  # forward
        self.reg_max    = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no         = num_classes + self.reg_max * 4  # number of outputs per anchor
        self.num_classes = num_classes
    ...
    def forward(self, x, gt_x):
        # feat = []
        x = checkpoint(self.rest, x)
        # x = self.rest(x)
        xpre = x
        #  backbone
        feat0, feat1, feat2, feat3 = self.backbone.forward(x)
        featc0, featc1, featc2, _ = self.clean_backbone.forward(gt_x)
        ...
        # del P4
        # del P5
        # del P3_downsample, P4_downsample, P4_upsample, P5_upsample
        ...
        # sobel 计算
        # gra0 = self.gra1(feat0, featc0)
        # gra1 = self.gra2(feat1, featc1)
        # gra2 = self.gra3(feat2, featc2)
        # print(gra0+gra1+gra2)
        # print(gra0.grad)

        # feat += [(feat0, featc0)]
        # feat += [(feat1, featc1)]
        # feat += [(feat2, featc2)]
        # [[feat0,feat1,feat2],[featc0,featc1,featc2]]

        return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device), [[feat0,feat1,feat2],[featc0,featc1,featc2]], xpre


class finYoloBody2(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(finYoloBody2, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
        ...
        # SobelConv
        self.gra1 = sobel_conv.Gradient(64)
        self.gra2 = sobel_conv.Gradient(128)
        self.gra3 = sobel_conv.Gradient(256)
    ...
    def forward(self, x, gt_x):
        ...
        return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device), gra0+gra1+gra2, xpre
...
class ReUnionYoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        super(ReUnionYoloBody, self).__init__()
        depth_dict          = {'n' : 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.00,}
        width_dict          = {'n' : 0.25, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        deep_width_dict     = {'n' : 1.00, 's' : 1.00, 'm' : 0.75, 'l' : 0.50, 'x' : 0.50,}
        dep_mul, wid_mul, deep_mul = depth_dict[phi], width_dict[phi], deep_width_dict[phi]
        ...
        #===================图像恢复模块===============#
        self.rest = LarKPN2(16)  # 16 为batchsize
        #-----------------------------------------------#
        #   输入图片是3, 640, 640
        #-----------------------------------------------#
        ...
        self.nl         = len(ch)
        # self.stride     = torch.zeros(self.nl)
        res = self.backbone.forward(torch.zeros(1, 3, 256, 256))
        self.stride     = torch.tensor([256 / x.shape[-2] for x in res[:3]])  # forward
        self.reg_max    = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no         = num_classes + self.reg_max * 4  # number of outputs per anchor
        self.num_classes = num_classes
    ...
    def forward(self, x):
        ...
        # origin_cls      = [xi.split((self.reg_max * 4, self.num_classes), 1)[1] for xi in x]
        dbox            = self.dfl(box)
        return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device), img
        # return x

# if __name__ == '__main__':

#     kpn = reDAYoloBody([256,480], 9, 's',False).cuda()
#     a = torch.randn(4, 3, 256, 480).cuda()
#     b = kpn(a, a)
#     print(b.shape)
#     # print(b.shape)