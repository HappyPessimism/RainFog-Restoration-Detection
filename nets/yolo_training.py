...
class Lossfin:
    def __init__(self, model, total_epoch): 
        self.bce    = nn.BCEWithLogitsLoss(reduction='none')
        self.stride = model.stride  # model strides
        self.nc     = model.num_classes  # number of classes
        self.no     = model.no
        ...
        self.n = 140
        # self.sobel = sobel_conv.SobelLossMultiChannel()
        self.sobel = nn.L1Loss()
    ...
    def __call__(self, preds, batch, epoch):
        ...
        if fg_mask.sum():
            loss[0], loss[2] = self.bbox_loss(pred_distri, pred_bboxes, anchor_points, target_bboxes, target_scores,
                                              target_scores_sum, fg_mask)
        s = epoch/self.total_epoch
        s_t = torch.tensor(s, dtype=torch.float32)
        # e_p = torch.exp(-s_t*s_t)
        # gra = 0
        # print(preds[5].mean().item())
        # for feat1, feat2 in preds[5]:
        #     # print(feat1.shape)
        #     gra += self.sobel(feat1, feat2)
        #     # print(gra)
        # gra = torch.sum([self.sobel(feat1, feat2) for feat1, feat2 in zip(preds[5][0], preds[5][1])])
        # gra = sum(self.sobel(feat1, feat2) for feat1, feat2 in zip(preds[5][0], preds[5][1]))
        feat_loss = sum(self.sobel(feat1, feat2) for feat1, feat2 in zip(preds[5][0], preds[5][1])) * torch.exp(-s_t*s_t)
        # print(feat_loss)
        ...
        # print(loss)
        if self.n == epoch:
            print(feat_loss.sum().item())
            self.n += 1 
        return loss.sum() + 0.05*feat_loss.mean()
...
class Loss2:
    def __init__(self, model): 
        ...
        # dict_new = self.backbone.state_dict().copy()
        # name_list = list(self.backbone.state_dict().keys())

        # cp = torch.load(back_path)
        # cp_dict = cp
        ...
        # self.backbone.load_state_dict(dict_new)
    ...
    def __call__(self, preds, batch, img_gt):
        # 获得使用的device
        device  = preds[1].device
        # box, cls, dfl三部分的损失
        loss    = torch.zeros(3, device=device)  
        ...
        ssim_loss = -criterion_ssim(img_gt, preds[5])
        smooth_loss = F.smooth_l1_loss(preds[5], img_gt)
        restore_loss = ssim_loss + smooth_loss
        ...
        loss[1] *= 0.5  # cls gain
        loss[2] *= 1.5  # dfl gain
        return loss.sum() + 0.5*restore_loss# loss(box, cls, dfl) # * batch_size
